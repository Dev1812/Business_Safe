...
map.to_top_showen = false;
var ToTop = {
  init: function() {
    addEvent(window, 'scroll', ToTop.onWindowScroll);
    addEvent('to_top', 'click', ToTop.onTopClick);
  },
  onWindowScroll: function() {
    if(getTopScroll() > 290) {
      show('to_top');
	  map.to_top_showen = true;
    } else if(map.to_top_showen === true) {
      hide('to_top');
	  map.to_top_showen = false;
    }
  },
  onTopClick: function() {
    ...
    if(getTopScroll() > 0) {
      request_frame(ToTop.onTopClick);
    }
  }
}
